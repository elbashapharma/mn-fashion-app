...
Future<int?> mergePendingOrdersToOneConfirmed(int customerId) async {
 ...
 // =========================
// Customer Statement (Ledger) - ALL movements
// types: order(debit) | payment(credit) | expense(debit) | cash(sign-based)
// =========================
Future<List<Map<String, Object?>>> customerLedger(int customerId) async {
  ...
  for (final r in orders) {
    out.add({
      ...
      "order_id": (r["order_id"] as num?)?.toInt(),
  ...
  // 3) Expenses for this customer => DEBIT
  final exps = await d.rawQuery("""
    SELECT
      created_at AS created_at,
      order_id AS order_id,
      amount_egp AS amount,
  ...
  for (final r in exps) {
    final expType = (r["exp_type"] ?? "") as String;
    ...
    out.add({
      ...
      "order_id": (r["order_id"] as num?)?.toInt(),
      "amount": (r["amount"] as num).toDouble(),
      "note": note.isEmpty ? "مصروف ($expType)" : note,
    });
  }

  // 4) Cash txns related to this customer (if you recorded customer_id) => sign-based
  // amount_egp: + دخل , - صرف
  ...
  for (final r in cash) {
    final cashType = (r["cash_type"] ?? "") as String;
    final note = ((r["note"] ?? "") as String).trim();
    out.add({
      "type": "cash",
      "created_at": (r["created_at"] as num).toInt(),
      "order_id": (r["order_id"] as num?)?.toInt(),
      "amount": (r["amount"] as num).toDouble(), // may be + or -
      "note": note.isEmpty ? "حركة خزنة ($cashType)" : note,
    });
  }
